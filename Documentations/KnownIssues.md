...
**关闭方式不满足本条自订的标准，重开为「限期收敛」，2026-08-16（第三轮 review）。**
上面写的关闭标准是「要么共享实现、要么加一条对照两份边界检查的测试——**注释不算
机制**」。实际的关闭动作是**又一次手工同步**：把缺的检查补回去。复核确认
`Tests/` 下既没有任何对照 `rawChildIndex` 两份实现的测试，也没有共享实现——**两个
选项一个都没做到**，只是把这一次的漂移修平了。同一条契约在同一条目上被违反第二次。

而且副本不止两份：`BufferView.childIndices(of:)` 里还有第三份同一分派规则的实现，
从未登记。它与另两份功能不同（收集全部子索引 vs 定位一个），但走的是同一套
`payloadKind` 分派，漂移风险同源。

维持「限期收敛」，标准不变（共享实现或对照测试，二选一，必须落地其一），并把第三份
副本纳入范围。一个未经验证的线索：`withSpans` 调 `UnsafeBufferPointer.span` 时没有
availability 门禁，所以共享实现可能只差一层薄封装——但 `Span` 存储的逃逸约束是否允许
直接共享**尚未验证**，收敛时先验这一点。收敛计划见
...
**维持原判，2026-08-16（第三轮 review 重提）。**
第三轮把这一条重新提了出来，理由是「N14 只证明了 head 与 main 行为一致，从未证明
字节计数 / 字素丢弃这个跨域是正确的」。**这个理由不成立**：它只读了上面第一段（驳
「F9 引入域错配」），漏了第二段——上面那句「组合音标会连同 `s` 一起被吃掉……不单独
处理」正是对这个行为本身的裁决，是**知情的 won't-fix**，不是没看见。

按本文件的契约，重提需要**新事实推翻理由**；第三轮提供的是同一行为的再次演示，无新
事实。理由今天依然成立：触发需要「前缀后紧跟组合标量」的畸形输入，两版一致，
`demangleAsNode` 对它必然失败，strip 输出砍 2 字节还是 4 字节都是垃圾进垃圾出。

补记两点（都不构成推翻，登记以免下次再被当成新发现）：
- `isSwiftSymbol` 对这种输入返回 `true`，所以「先判 `isSwiftSymbol` 再 strip」的调用
  方拿到的是被多砍了字节的结果。要推翻本裁决，需要主张的是「`isSwiftSymbol == true`
  的输入应保证 strip 的字节保真」这条契约——第三轮没有提出这条主张。
- 把 `dropFirst` 换到 UTF-8 域是一行的概念债清理（nice-to-have），与本裁决不冲突。
