...
> **2026-08-02 更正：本节所述的下调已全部回退，三个常数恢复为上游值。**
> 下调依据的是「实测过的最深真实符号是 41 层」这一语料结论，而下游消费方报告：
> 普通的 SwiftUI 及同类泛型密集模块在 512 的打印上限下会输出 `<<too complex>>`。
> 也就是说该语料结论不成立，下调不是「削掉理论能力」而是在截断本该正确的输出。
> 前提被证伪后，同批同论证的另外两个常数一并回退。详见 `KnownIssues.md` 第二部分 N8。
>
> 下方的崩溃边界实测数据本身仍然有效，保留作为**尚未解决**的栈安全问题的依据
> （见 `KnownIssues.md` 第 4 条）——它是一个栈安全问题，要当作栈安全问题解决，
> 不能靠静默截断 release 构建能正常渲染的输出来换。

原方法：8MB 线程、debug 构建、嵌套 `Optional` 符号逐深度探测崩溃边界，取约 30% 安全余量。

| 引擎 | 上游常数（**现行**） | 实测崩溃边界（debug、8MB 线程） | 曾短暂下调至 | 下调后覆盖的嵌套层数 |
|---|---|---|---|---|
| `DemanglingPrinter.maxPrintDepth` | **768** | 725 层过 / 745 层崩 | ~~512~~（已回退） | ≈253 层 |
| `Remangler.maxDepth` | **1024** | 深度 565 过 / 605 崩（140/150 层，每层 4 深度单位） | ~~384~~（已回退） | ≈94 层 |
| `TypeDecoderEngine.maxDepth` | **1024** | 深度约 250 崩（120/130 层，每深度单位约 30KB！） | ~~160~~（已回退） | ≈78 层 |

**教训**：这批常数的校准建立在一次语料扫描给出的「最深 41 层」上，而那次扫描的覆盖面
不足以代表下游真实负载。再次下调任何一个上限之前，必须先有来自下游工作负载的语料证据。
...
- **深度上限维持上游值**（768 / 1024 / 1024）：曾下调至 512 / 384 / 160，因下游在 512 下实测 `<<too complex>>` 而全部回退（`KnownIssues.md` N8）。debug 构建下「先崩后拒」的风险因此仍在，作为独立的栈安全问题追踪（`KnownIssues.md` 第 4 条）。
