...
- `evolution/` — 演进提案（设计意图 + 决策日志）。`0001-node-store-arena.md` 是 `NodeStoreArena.md` 的提案原文；`0002-stack-safety.md` 是 `StackSafety.md` 的提案原文；`0003-review-hardening.md` 是 PR #6 review 收尾轮——冻结移交的 `NodeBuilder`（环不可构造）、整树重建的按图计价（memo 保共享）、扁平 `Codable` 格式。
