...
- **`StackBudget` / `StackSafeExecutor`** (`Utils/`) — recursion in the three engines is bounded by **remaining stack bytes**, not by a frame count (design notes and measurements in `Documentations/StackSafety.md`; proposal 0002). A frame count is a proxy for stack bytes and the two are not proportional across build configurations, `DemanglingNode` specializations, or thread stack sizes — the old fixed limits (`maxPrintDepth` 768, `Remangler.maxDepth` 1024) simultaneously let debug builds overflow an 8MB stack and rejected legitimate generics beyond ~384 nesting levels. `StackBudget.forCurrentThread()` is captured at each engine entry (`printRoot` / `mangle(_:)` / `TypeDecoderEngine.init`) and probed **on every level**; sampling every N levels is unsafe because a wrong per-level estimate crosses the whole reserve between probes. The probe is non-sticky (one deep path degrades, siblings do not) and the surviving frame-count constants are cycle backstops only. `StackSafeExecutor` moves work onto a **bounded pool of long-lived 64MB-stack workers**: workers never retire (an idle timeout has a window where a submission counts a retiring worker as available and the item is silently dropped), the pool is capped at `activeProcessorCount`, and a worker never submits back into the pool (thread-local flag → nested calls run inline, since printing re-enters demangling for nested mangled names). `withLargeStack(_:)` is the batch-boundary entry — wrap a whole indexing pass in it and every call inside runs inline with zero thread hop. **Adding a new recursion**: it must either converge on a guarded entry point or be iterative. Verify with the call-graph audit described in `Documentations/StackSafety.md` (build the SCCs, then delete the candidate convergence point and re-run) — a single check on `mangle` was never enough; six cycles bypassed it. Whole-tree walks outside the engines (`NodeCache.internTreeUnsafe`, `NodeStoreBuilder.internTree`, `Node.==` / `hash(into:)`, `NodeStore.materializeNode`, `NodeReference.structurallyEquals` / `structuralHash`) are iterative for the same reason, and `Node.deinit` tears trees down with an explicit work list because releasing a deep tree recurses in the runtime where no engine guard can reach it. **Still recursive and deliberately so**: `Node.Rewriter.rewrite` (an `open class` — converting it would change which points a subclass can override), `Node+CustomStringConvertible`'s `printNode` (backs `description`), `Demangler`'s `setParentForOpaqueReturnTypeNodesImpl` / `demangleBoundGenericArgs`, and the `demangleSwift3*` recursive-descent component (reachable only through the `_T` prefix). All run on a pooled 64MB worker, so they do not crash at any depth a real symbol reaches, but they fail by crashing rather than by erroring — do not raise the depth they are expected to survive without guarding them first. Where no stack measurement is available (non-Darwin, or `pthread_get_stackaddr_np` failing), `StackBudget` falls back to the frame count each engine used before it existed, so those platforms keep exactly their old behaviour instead of losing the guard.
