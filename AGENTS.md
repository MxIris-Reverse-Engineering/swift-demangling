...
- **`SymbolStore` / `SymbolStoreBuilder` / `NodeReference`** (`Store/`) — Arena-based compact storage for bulk demangling (evolution proposal 0001). Nodes are flat 12-byte `CompactNode` values referenced by `UInt32` indices in one contiguous buffer; the `~Copyable` builder hash-conses on insert and `consuming freeze()` produces an immutable `Sendable` store. `NodeReference` is a 16-byte value handle mirroring `Node` accessors (kind/text/index/children). `materialize()` rebuilds a standalone (non-`NodeCache`) `Node` tree with an index-keyed memo, so store-level subtree sharing survives as shared instances instead of expanding the DAG. The builder's `demangle(_:)` currently bridges through a transient `Node` tree (`internsSubtrees: false`); Phase 3 will parse directly into the arena.
- **`DemanglingNode` / `DemanglingPrinter`** (`Store/DemanglingNode.swift`, `Node/Printer/NodePrinter.swift`) — Phase 2 read-only tree protocol conformed by both `Node` and `NodeReference` (members named to match `Node`'s API so the generic printer body is representation-agnostic). The printer engine is `DemanglingPrinter<Target, SomeNode: DemanglingNode>`; the public `NodePrinter<Target>` is a thin facade over `DemanglingPrinter<Target, Node>`, and `NodeReference.print(using:)` prints straight from the store with **zero materialization** (verified byte-identical to the `Node` path across the 49k-symbol corpus × default/simplified/synthesizeSugar). When abstracting more consumers, name protocol members to match `Node`'s and note that `NodePrintContext.node` stays a concrete `Node?` (store path passes `name as? Node` → nil). The derived helpers (`isSimpleType`, `needSpaceBeforeType`, `isIdentifier(desired:)`, `isSwiftModule`) live **only** on the `DemanglingNode` extension — do not re-add copies on `Node`: the generic engine statically dispatches to the extension, so a parallel concrete copy would silently drift.
...
Sources/Demangling/
  ...
  Store/             — CompactNode, SymbolStore, SymbolStoreBuilder, NodeReference, DemanglingNode (evolution proposal 0001)
