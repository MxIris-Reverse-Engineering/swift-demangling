...
| 日期 | 变更 | 说明 |
|------|------|------|
| 2026-08-09 | 同族第四次露头，清点范围扩大并修复（PR #7 review F1） | 本提案处理的是**常量折叠**型整数陷阱（三个字面量拼写 + 源码扫描守卫）；review 证实同族的**运行时值**型一直漏网——`Demangler.swift` 从未被 KnownIssues §1 清点过，六处对攻击者可控数字「先窄化/自增、后检查」的拼写可被畸形输入触发 SIGTRAP（`try?` 拦不住），`Remangler.swift` 的 substitution 哈希另有一处。全部改为无符号域 `require` 界检查后再转换；行为守卫 exit test + 拼写守卫扫描测试双防线入库（互补盲区已在测试注释言明）。完整清点与修复记录见 `Documentations/KnownIssues.md` 2026-08-09 更新。 |
| 2026-08-09 | F1 补遗：第一轮横向排查漏掉的 Swift 3 环绕族（review 会话核实时发现） | 首轮按「窄化转换」特征扫同类，而 F1 头号站点的缺陷本质是「环绕算术后自增」——`demangleSwift3Index` 内部 `readInt() + 1` 与三个调用点的再 `+ 1` 不含任何窄化，按错误特征扫必然漏；review 会话在闭环核实中实测四条 `_Ttq…` 触发串全 SIGTRAP（main 既有，Initial commit 即如此）。四处同形补修（无符号域 `require` 后自增），触发串进 exit test，拼写守卫扩列并把正确的排查特征（「任何吃 `conditionalInt()`/`readInt()` 结果的算术」）写进测试注释。这是元模式（验证方法对某类问题结构性失明）在**排查本身**上的应验：用表面特征圈定「同类」，圈出来的就不是同一类。 |
