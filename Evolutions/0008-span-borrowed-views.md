...
| 2026-08-09 | PR #7 review F12 修复：seam 从「运行中可翻的进程开关」退役为「启动时定死的进程属性」 | `DualPathParityTests` 翻 `forcesLegacyPath` 做双腿对比，而 `.serialized` 只排序本套件内部——持有期间所有并行套件被静默拽上旧路径，现代路径覆盖非确定性丢失（4d08c9a 修过「本套件被外部 seam 影响」的方向，没修「本套件影响别人」）。修复：旧腿抽为 internal 直接入口 `demangleAsNodeOnLegacyRuntimePath`，套件显式驱动双腿、全程不碰进程状态并撤 `.serialized`；task-local 方案被否（seam 在 StackSafeExecutor 闭包内读取，pthread hop 不传播 task-local）；扫描测试钉住测试 target 无 seam 赋值点（防线盲区：对经助手洗过的赋值失明，已在测试注释言明）。env 双跑纪律不变。 |
| 2026-08-09 | PR #7 review F11 修复：store 侧文本物化接入 seam + 有效性闸门 | `BufferView.text` 只看 `#available`，`DEMANGLING_FORCE_LEGACY_PATH` 双跑在 store 侧跑的是同一条路——`String(decoding:)` 这条「26 之前全部部署目标实际执行的分支」从未被执行过；且 `UTF8Span(unchecked:)` 无任何有效性闸门，错误但在界内的跨 store index 可切开非 ASCII 条目伪造非法 String。修复：① store 创建时快照 seam（每物化读全局锁太贵；env 在进程启动设定，先于一切 store）；② builder 增量维护**全表 ASCII 位**随每次发布进视图——全 ASCII 表上任意界内子区间自成合法 UTF-8（与 demangler 侧 `isKnownASCII` 闸门同构），非 ASCII 表（punycode 场景）降级带校验解码。`#available` 分流点清单入 `SpanBorrowedViews.md`（防线盲区自答：双跑纪律对「没接进 seam 的分支」失明，清单是登记义务）。 |
| 2026-08-09 | PR #7 review F13 修复：benchmark 测量窗口互斥 + `malloc_logger` 保存恢复 | 三个 benchmark 套件共用一个 gate 而 `.serialized` 只管套件内——窗口并行交错，一个套件的 `start()` 清另一个的活计数器，两个并发 footprint 采样器互认峰值；`CMallocCounter` 覆写 `malloc_logger` 不保存原值、退出写 0（此后整个进程的 MallocStackLogging/Instruments/`leaks` 全废），且为非原子裸写。修复：跨套件 `ExclusiveMeasurementWindow.run { }` 互斥（语料装载留锁外），钩子保存/恢复前值并改 `__atomic` 读写，采样线程启动挪出 malloc 窗口（**停止顺序保留计数器先停**——与 review 建议相反，理由：采样器 stop 的线程 join 会把分配砸回窗口，而 footprint 峰值对停止顺序不敏感）；`MeasurementToolbox.md` 增补「新套件必须包窗口」规则。受污染数字的重测与 0009/0010 订正随本批 benchmark 重跑落地（见各自决策日志）。 |
| 2026-08-09 | 干净窗口重测（F13 之后）：分配账目再确认，吞吐留待空载 | 首次无重叠窗口的 benchmark 重跑：mallocs/symbol **85.8（transient demangle）/ 110.4（store 构建）与既有记录逐位一致**——分配账目从未被窗口重叠污染到比值层面（重叠影响的是绝对计数被中途拔钩的场景，见 0010 本日订正行）。吞吐：transient 73,851 / store-build 48,711 / store-print default 121,464 · simplified 156,434 · sugared 95,424 symbols/s——**本轮不与历史比较**：机器连续满载约一小时后 sugared 三遍差 ±17%，不满足空载纪律；若需评估 review F1/F6/F7 新增界检查的吞吐代价，须另行空载 A/B。store-build 48,711 高于历史 44,982-45,098 这一事实至少说明界检查不构成显性回归。 |
| 2026-08-09 | PR #7 review 补充发现 2/8 落地（测量与测试基建小修） | **补充发现 2**：`PhysicalFootprintSampler.stop()` 无匹配 `start()` 时在无人 signal 的信号量上永久阻塞——补一行守卫改为返回 0（测量装置的误用应可诊断，不应挂死测试进程）。**补充发现 8**：`EmbeddedFlavorTests` 只构造 `Demangler<ArrayWordRanges>`（legacy 词表配置），默认入口在 macOS 26 上实际实例化的 `InlineWordRanges` 配置在该套件零覆盖——helper 改为现代运行时下两种配置都跑并断言 flavor 一致。其余补充发现的裁决见 `KnownIssues.md` N9–N13（#1 precondition 收口 / #3 误报 / #6 #7 #9 不修留档），#4（`SharedNodeStore` 缺 `reference(at:)`）为真实 API 缺口但无当前消费方，按提案制留维护者决定。 |
