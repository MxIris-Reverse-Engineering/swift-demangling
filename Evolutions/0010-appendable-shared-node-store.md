...
| 2026-08-08 | 实景复测回填（验收闭环） | 下游迁移（MachOSwiftSection `feature/node-store-migration` @ d96520f：`InternedNodeReferenceCache` 退桶层、`TypeDefinition` 字段树进镜像 store、`lateDemangledNode` 自持 side store）落地后，RV 五镜像 memory graph 复测（用户亲抓 Xcode memory graph；RV Debug 构建经 sibling 符号链接解析到本分支 bb1f81c + 迁移分支 d96520f；image 组合与基线一致：Foundation + libswiftCore + AppKit + SwiftUI + SwiftUICore）：**`NodeStore` 实例 14,451 → 15（−99.9%）**；class `Node` 208,809 → 207,489（−0.6%，预期内——0010 合并的是 store 实例，`Node` 本体属 `NodeCache` 路径，小幅下降来自 0011 同轮的 transient 切换）。相对本提案验收标准「≤ 10」多出的数个实例来自下游迁移设计的两处知情调整（`lateDemangledNode` 每 scope 自持 side store、per-process 作用域外壳），其设计文档已留档——意图形状「每镜像一个 + 少量自持 side store」达成。至此验收三项全部闭环：正确性（corpus 对拍零差异 + TSan 绿）、性能（冻结路径持平或更优）、内存（实景 −99.9%）。 |
| 2026-08-09 | PR #7 review F4 落地：共享 store 语料对拍不再吞失败 | `SharedStorePrintParitySweep` 原以 `guard let … try? … else { continue }` 在比较前丢弃失败符号，验收数字对「成功变失败」的回归不可见。改造同 0008 的 print-parity sweep：单边失败=mismatch、双边失败按 stdlib 裁判分类并断言无回归。重跑：439,522 符号 × 3 选项集 × 双运行时路径 0 mismatch、retiredBuffers=0 不变，另有 11 个 stdlib 同拒符号首次显形（与 0008/0011 的同一批 11 个）。 |
| 2026-08-09 | PR #7 review 第 2 步落地（F5/F6/F7/F8，同批 commit） | **F5（维护者裁决方案 A）**：空代也进退休保活链——2026-08-08「空代直接释放（不可能被寻址）」的论证只覆盖**解引用**，而已发布描述符（及其零长 Span）的**形成**就记录了空代的 base 地址；该论证由本行取代，`SharedViewState` 的「任何已获得的描述符终生有效」承诺自此无条件成立（代价：每空代一个元素的分配）。红→绿测试 `emptyGenerationsAlsoEnterTheRetirementChain`（含「两次预留之间取得的引用增长后仍可读」行为钉）；`reservationKeepsTheRetirementChainEmpty` 相应更名为 `reservationRetiresOnlyTheInitialEmptyGenerations`（足量预留的退休链 = 恰好三个初始空代，intern 期间零增长的契约不变）。**F6**：builder 探针的四处 range 读从裸 `UnsafeBufferPointer` 下标（仅 debug 检查）收敛到新的 `withBuffer(in:)` 界检查访问器——release 越界读的后果不是崩溃而是拿未初始化容量区参与相等比较、把两棵树别名到同一 index 的静默数据损坏；`StoreBuffer` 文档措辞同步修正，exit test `rangeReadPastTheInitializedCountTraps` 入库。**F7**：`internInterior` 的自底向上不变量 `assert` → `precondition`——注释自称承载 step 4 的 stale-view 内存安全，这是 release 配置的主张，守卫必须在 release 生效；`internInterior` 是 interior 节点唯一铸造漏斗（步骤 2 已确认），many-children 路径本就遍历 `childIndices`，成本近零。**F8**：`reserveCapacity` 增长取加倍下界 `max(minimum, capacity * 2)`——精确扩容（步骤 1 记为优点）+ 终生保活的退休链叠加成增量预留 O(k²) 驻留；首次预留自零容量仍精确（一次性预留流程的利用率性质不变），只有增量再预留付出至多 2× 超配换 O(log) 代数。策略测试 `incrementalReservationsRetireBoundedGenerations`（断言加倍下界使坡道第 4/6/7/8 步为 no-op，修复前每步都增长）。**吞吐复测挂账**：F6/F7 在构建热路径上各加一道 precondition，量级预期在噪声内，但按纪律数字要等 F13（benchmark 测量窗口互斥）修复后一并重测——用被污染的窗口测出来的数字没有意义。 |
| 2026-08-09 | PR #7 review F10 落地（维护者裁决：改签名、编译期暴露） | 步骤 1 把 `NodeReference.textUTF8` 从零拷贝切片改成拷贝桥时，commit 与文档都只宣告了「拷贝」，漏了**索引基**从 store 字符串表偏移变 0 基——签名未变，下游把切片索引与字符串表关联的代码静默错读，且当时的测试全用 `Array(...)` 归一化，恰好抹掉这个差异。三种修法（恢复绝对索引 / 改签名 / 只宣告）中维护者选改签名：更名 `textUTF8Bytes`、返回类型改 `[UInt8]?`——拷贝与 0 基都进了签名，旧拼写编译失败迫使下游读新文档。内部三处使用点同批迁移；不做 `Array` 归一化的内容钉测试入库（表中第二条文本，即两种基分歧的情形）。 |
| 2026-08-09 | 干净窗口重测：步骤 5 的 mini-stores malloc 数字订正 | F13 修复后的首次无重叠窗口重跑增量基准：**mini-stores 模式 892,084 次分配事件，而步骤 5 记录的 616,023 是被窗口重叠少计的**——当时三个 benchmark 套件并行，另一套件的 `stop()` 会把进程级 `malloc_logger` 拔掉，计数在窗口中途静默停止（review F13 的直接证据；shared 模式 308,047 与原记录 308,046 吻合，说明那一窗恰好未被打断）。订正后的对比：mini-stores 892,084 vs shared 308,047（**−65%**，优于原记录的 −50%）；store 实例 14,000 → 1、构建 best 0.058s → 0.034s（−41%，与原 −43% 同带）不变。retiredBuffers 现为 4（F5 后初始空代进链 + 一次加倍增长），与新契约一致。按惯例原记录保持原貌，以本行订正。吞吐类数字的空载复测保留意见同 0009 本日行。 |
| 2026-08-09 | PR #7 review 第 5 步落地（F14/F15 + 补充发现 5，同批 commit） | **F14**：结构 walk 全部收敛为「一次 walk 一次视图解析」——`structurallyEquals(_ node:)` 与 `structuralDigest()` 原在 `withSpans` 内逐节点调 `store.contents(of:)`/`store.indexPayload(of:)`，共享 store 上每次都重进锁槽、且一次比较的两半可能读到不同视图；现改经单次 `withView` 解析的视图取值（跨引用重载经查本就零重解析）。`reference(at:)` 的边界检查改读已解析视图，消除每次铸引用的第二次锁拷贝。**原子发布描述符判定暂不做并留档**：提案 214-215 行设想的「描述符切换才取锁」形态，在「每次 intern 都发布」的现实下需要 seqlock/双字 DCAS 级设计（48 字节描述符无法原子加载，逐缓冲 128 位原子会引入跨缓冲撕裂窗口），而 RV 实景剖析未见此锁为热点、walk 级单次解析已覆盖全部热路径——按「实现取锁不构成热点」的原判据，为无实证收益引入撕裂风险不成立；若未来剖析翻案再立项。**F15**：异步 `print` 补上与同步版对齐的 `withExtendedLifetime(store)` 锚点（退休链时代「store 强活」的含义已从一块 buffer 扩大为整条链，不应赌优化器保留闭包捕获到最后一刻）。**补充发现 5**：`publishCurrentState` 的每 intern 一次 `[AnyObject]` 字面量分配消除——`SharedViewState.State` 的保活锚拆为三个存储字段，无增长的发布只做恒等比较零分配。 |
