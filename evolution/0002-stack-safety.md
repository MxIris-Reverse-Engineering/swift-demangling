...
## 修订：三个深度上限回退到上游值（2026-08-02）

上一节「新定」确立的三个常数（printer 512、remangler 384、TypeDecoder 160）**已全部回退**
为上游值 768 / 1024 / 1024。

**推翻原决策的证据**：下游消费方报告，普通的 SwiftUI 及同类泛型密集模块在 512 的打印
上限下会输出 `<<too complex>>`。原校准的两个支柱之一是「实测过的最深真实符号是 41 层，
所有常数都有 2 倍以上余量」——这个语料结论被证伪了。真实符号确实会超过 512，因此下调
的实际效果不是「release 下拒绝点比上游早」，而是**截断本该正确的输出**。

**为什么三个一起回退，而不只回退 printer**：三个常数出自同一批、同一套论证、同一次
语料测量。其中一个的前提被证伪，另外两个的前提并没有独立证据支撑——TypeDecoder 的
160 尤其如此，它是三刀里最狠的（6.4×），而 4.5M 符号语料**从不调用** TypeDecoder，
等于完全没有语料验证。留下前提已倒的校准值比回退更难解释。

**未解决的代价**：下调本来要解决的 debug 构建爆栈风险是真实的，回退后它也回来了，且
比 512 时更宽——768 层 × 每层约 11.6KB ≈ 8.9MB，**超过池线程自身的 8MB**，于是除了
原有的「半耗尽栈」窗口外，新增一个「即使跳到全新 8MB worker 仍会在约 725 层爆栈」的
窗口。这条作为独立的栈安全问题追踪于 `KnownIssues.md` 第 4 条（已按新常数重写），
首选修法是**按调用时实际剩余栈折算本次生效的上限**——栈少就早点 `<<too complex>>`，
栈多就走满 768，既堵住两个窗口又不牺牲 release 构建的能力。

**流程教训**：常数校准依赖的语料扫描（「最深 41 层」）覆盖面不足以代表下游真实负载，
而语料本身（4.5M dyld cache 符号）看起来足够权威，掩盖了这一点。**再次下调任何一个
深度上限之前，必须先有来自下游工作负载的语料证据**；这句话已写入三处常数各自的代码
注释、`AGENTS.md`、`Documentations/StackSafety.md` 与 `KnownIssues.md` N8。
